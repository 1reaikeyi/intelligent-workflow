...
public class SetmealTool {
    ...
    public List<Setmeal> queryByMultiCondition(
        ...
        try {
            LambdaQueryWrapper<Setmeal> wrapper = new LambdaQueryWrapper<>();
            // ID精确查询
            if (setmealQuery.getId() != null) {
                wrapper.eq(Setmeal::getId, setmealQuery.getId());
            }
            // 分类ID精确查询
            if (setmealQuery.getCategoryId() != null) {
                wrapper.eq(Setmeal::getCategoryId, setmealQuery.getCategoryId());
            }
            // 名称模糊查询
            if (setmealQuery.getName() != null && !setmealQuery.getName().isEmpty()) {
                wrapper.like(Setmeal::getName, setmealQuery.getName());
            }
            // 价格区间查询
            if (setmealQuery.getMinPrice() != null) {
                wrapper.ge(Setmeal::getPrice, setmealQuery.getMinPrice());
            }
            if (setmealQuery.getMaxPrice() != null) {
                wrapper.le(Setmeal::getPrice, setmealQuery.getMaxPrice());
            }
            // 状态精确查询
            if (setmealQuery.getStatus() != null) {
                wrapper.eq(Setmeal::getStatus, setmealQuery.getStatus());
            }
            // 描述模糊查询
            if (setmealQuery.getDescription() != null && !setmealQuery.getDescription().isEmpty()) {
                wrapper.like(Setmeal::getDescription, setmealQuery.getDescription());
            }
            return setmealMapper.selectList(wrapper);
        } catch (Exception e) {
